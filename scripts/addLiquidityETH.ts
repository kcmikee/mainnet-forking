...
async function main() {
  ...
  const LP_ETH_Contract = await ethers.getContractAt(
    "IERC20",
    ETH_USDC_PAIR,
    impersonatedSigner
  );
  ...
  const ETHUSDCBalBefore = await LP_ETH_Contract.balanceOf(
    impersonatedSigner.address
  );
  ...
  console.log(
    "LP ETH token balance before liquidity",
    Number(ETHUSDCBalBefore)
  );
  ...
  const ETHUSDCBalAfter = await LP_ETH_Contract.balanceOf(
    impersonatedSigner.address
  );
